...
/*********************/
	...
	// Determine base URL, settings
		...
		y = x.attr('src').split('?');
		baseURL = y[0].replace(/js\/init\.js/, '');
		// Override settings
			if (y.length > 1)
			{
				x = y[1].split('&');
			...
			if (jQuery.browser.msie)
			{
				// HTML5 Shiv
					if (jQuery.browser.version < 9)
						_head.append('<script type="text/javascript" src="' + baseURL + 'js/html5shiv.js" />');
	...
	// Insert stylesheets
		headQueue.pushE_5grid(baseURL + 'css/core.css')
		headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '.css');
...
/*********************/
	...
	(function() {
		...
		if ((doMobile && (ww <= _settings.breakpoint_mobile || sw <= _settings.breakpoint_mobile)) || doMobileOnly)
		{
			...
			headQueue.pushE_5grid(baseURL + 'css/core-mobile.css');
			...
			if (_settings.mobileUI == 1)
			{
				...
				{
					headQueue.pushE_5grid(baseURL + 'css/mobileUI-' + _settings.mobileUI_theme + '.css');
			...
			headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '-mobile.css');
		}
		else
		{
			// Fluid (exclusive)
			if (doFluid)
			{
				...
				headQueue.pushE_5grid(baseURL + 'css/core-desktop.css');
				headQueue.pushE_5grid(baseURL + 'css/core-fluid.css');
				headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '-fluid.css');
			}
			// Desktop
			else if (doDesktop)
			{
				...
				headQueue.pushE_5grid(baseURL + 'css/core-desktop.css');
				headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '-desktop.css');
				...
				if (ww >= _settings.breakpoint_1000px)
				{
					...
					_head.prepend('<meta name="viewport" content="width=' + _settings.viewport_is1200px + '" />');
					headQueue.pushE_5grid(baseURL + 'css/core-1200px.css');
					
					// Load 1200px stylesheet if 1200px was explicitly enabled
					if (do1200px)
						headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '-1200px.css');
				}
				// 1000px
				else
				{
					...
					headQueue.pushE_5grid(baseURL + 'css/core-1000px.css');

					// Load 1000px stylesheet if 1000px was explicitly enabled
					if (do1000px)
						headQueue.pushE_5grid(baseURL + 'css/' + _settings.prefix + '-1000px.css');
				}
			}
			else
			{
