...
/*********************/
	...
	// Determine base URL, opts
		x = jQuery('script').filter(function() { return this.src.match(/js\/init\.js/); }).first();
		y = x.attr('src').split('?');
		_baseURL = y[0].replace(/js\/init\.js/, '');
		_opts = new Array();
...
/*********************/
		...
		if (jQuery.browser.msie)
		{
			// HTML5 Shiv
				if (jQuery.browser.version < 9)
					_head.append('<script type="text/javascript" src="' + _baseURL + 'js/html5shiv.js" />');
	...
	// Insert stylesheets
		_headQueue.pushE_5grid(_baseURL + 'css/5grid/core.css')
		_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '.css');
...
/*********************/
	...
	(function() {
		...
		if ((_mobile && (ww <= 480 || sw <= 480)) || _mobileOnly)
		{
			...
			_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-mobile.css');
			...
			if (_opts['mobileUI'] == 1)
			{
				...
				{
					_headQueue.pushE_5grid(_baseURL + 'css/mobileUI-' + _opts['mobileUI.theme'] + '.css');
			...
			_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '-mobile.css');
		}
		else
		{
			// Fluid (exclusive)
			if (_fluid)
			{
				...
				_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-desktop.css');
				_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-fluid.css');
				_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '-fluid.css');
			}
			// Desktop
			else if (_desktop)
			{
				...
				_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-desktop.css');
				_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '-desktop.css');
				...
				if (ww >= 1200)
				{
					...
					_head.prepend('<meta name="viewport" content="width=' + _opts['viewport.is1200px'] + '" />');
					_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-1200px.css');
					
					// Load 1200px stylesheet if 1200px was explicitly enabled
					if (_1200px)
						_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '-1200px.css');
				}
				// 1000px
				else
				{
					...
					_headQueue.pushE_5grid(_baseURL + 'css/5grid/core-1000px.css');

					// Load 1000px stylesheet if 1000px was explicitly enabled
					if (_1000px)
						_headQueue.pushE_5grid(_baseURL + 'css/' + _opts['prefix'] + '-1000px.css');
				}
			}
			else
			{
